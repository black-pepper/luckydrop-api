...
create table public.participant_lists (
  id bigserial not null,
  ...
  updated_at timestamp with time zone not null default now(),
  deleted_at timestamp with time zone null,
  constraint participant_lists_pkey primary key (id),
  constraint participant_lists_user_id_fkey foreign key (user_id) references public.users (id)
);
...
create index if not exists idx_participant_lists_user_id on public.participant_lists using btree (user_id);
